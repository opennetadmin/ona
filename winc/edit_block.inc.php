...
function ws_editor($window_name, $form='') {
    ...
    if (is_numeric($form['block_id'])) {
        ...
        $block['IP_BLOCK_START'] = ip_mangle($block['ip_addr'], 'dotted');
        $block['IP_BLOCK_MASK'] = ip_mangle($block['ip_mask'], 'dotted');
    }


    // Escape data for display in html
//    foreach(array_keys($block) as $key) { $block[$key] = htmlentities($block[$key], ENT_QUOTES); }
...
function ws_save($window_name, $form='') {
    ...
    // Validate input
    if (!$form['description'] or !$form['start'] or !$form['mask'] or !$form['summarization']) {
    ...
    if ($form['block_id']) {
        ...
        $form['set_mask'] = $form['mask'];
        $form['set_summarization'] = $form['summarization'];
