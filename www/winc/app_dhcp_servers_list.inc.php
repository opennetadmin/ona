    ...
    <table width="100%" cellspacing="0" border="0" cellpadding="0" >
        <tr>
            <td id="{$form_id}_{$tab}_tab" nowrap="true" class="table-tab-active">
                DHCP Servers <span id="{$form_id}_{$tab}_count"></span>
            </td>
...
function ws_display_list($window_name, $form) {
    ...
    <table cellspacing="0" border="0" cellpadding="0" width="100%" class="list-box">
        ...
        <tr>
            ...
            <td class="list-header" align="center" style="{$style['borderR']};">Subnet count</td>
            <td class="list-header" align="center">&nbsp;</td>
        </tr>

EOL;

    $where = 'id in (select host_id from dhcp_server_subnets group by host_id)';
    ...
    foreach ($records as $record) {
        ...
        list($status, $usage_rows, $tmp) = db_get_records($onadb, 'dhcp_server_subnets', "host_id = {$record['id']}", '', 0);
        ...
        <tr onMouseOver="this.className='row-highlight'" onMouseOut="this.className='row-normal'">
            ...
            <td class="list-row">
                <a title="View DHCP server. ID: {$record['id']}"
                   class="act"
...
EOL;
    ...
    <div class="act-box" style="padding: 2px 4px; border-top: 1px solid {$color['border']}; border-bottom: 1px solid {$color['border']};">
        <form id="{$form['form_id']}_add_dhcp_srv_{$record['id']}"
                ><input type="hidden" name="js" value="{$refresh}"
        ></form>
        <!-- ADD server LINK -->
        <a title="New DHCP server"
            class="act"
            onClick="xajax_window_submit('edit_dhcp_server', xajax.getFormValues('{$form['form_id']}_add_dhcp_srv_{$record['id']}'), 'editor');"
