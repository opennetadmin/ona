...
EOL;
    ...
    $response->assign("{$form['form_id']}_employees_count",  "innerHTML", "({$count})");
...
function ws_delete($window_name, $form='') {
    ...
    if ($_SESSION['auth']['client']['url'] == 'demo') {
        $response = new xajaxResponse();
        $response->script("alert('Feature disabled in this demo!');");
        return $response;
    }
    ...
    if ($_SESSION['auth']['user']['id'] == $form) {
        $response = new xajaxResponse();
        $response->script("alert('Sorry, but you can\\'t delete your own admin account!');");
        return $response;
    }
