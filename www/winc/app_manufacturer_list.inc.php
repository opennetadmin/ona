...
EOL;
    ...
    <div class="act-box" style="padding: 2px 4px; border-top: 1px solid {$color['border']}; border-bottom: 1px solid {$color['border']};">
        ...
        <a title="New manufacturer"
            class="act"
            onClick="xajax_window_submit('app_manufacturer_edit', 'js=> ', 'editor');"
...
EOL;
    ...
    $response = new xajaxResponse();
    $response->assign("{$form['form_id']}_manufacturer_count",  "innerHTML", "({$count})");
...
function ws_delete($window_name, $form='') {
    ...
    list($status, $rows, $manufacturer) = db_get_record($onadb, 
                                                ...
                                                array('id' => $form));
    if ($status or !$rows) {
        $response->script("alert('Delete failed: Manufacturer id {$form} does not exist');");
        return $response;
    }
